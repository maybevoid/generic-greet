...
mod v3 {
    ...
    impl FormalPerson {
        pub fn new(salutation: &str, first_name: &str, last_name: &str) -> Self {
            Self {
    ...
    impl HasName for FormalPerson {
        fn name(&self) -> String {
            format!("{} {} {}", self.salutation, self.first_name, self.last_name)
        }
    }
    ...
    pub fn greet_many_generic<Person: HasName>(persons: &Vec<Person>) -> Vec<String> {
        persons.iter().map(greet_generic).collect()
    }
}

mod v4 {
    use crate::v3::{Anonymous, CasualPerson, FormalPerson, HasName};
...
mod v5 {
    ...
    pub type AnyPersonGeneric = Either<FormalPerson, Either<CasualPerson, Anonymous>>;

    pub struct AnyPerson(pub Either<FormalPerson, Either<CasualPerson, Anonymous>>);
...
mod v6 {
    ...
    pub trait Greeter {
        fn greet(&self, person: &impl HasName) -> String;
    }
...
mod v7 {
    use crate::v3::{Anonymous, CasualPerson, FormalPerson};
    use crate::v5::{AnyPerson, AnyPersonGeneric, Either};

    pub trait Greeter<Person> {
        fn greet(&self, person: &Person) -> String;
    }

    fn greet_many<P, G: Greeter<P>>(greeter: &G, persons: &Vec<P>) -> Vec<String> {
        persons.iter().map(|person| greeter.greet(person)).collect()
    }
    ...
    where
        ...
        G: Greeter<B>,
    {
        fn greet(&self, person: &Either<A, B>) -> String {
            match person {
    ...
    impl Greeter<FormalPerson> for CustomGreeter {
        fn greet(&self, person: &FormalPerson) -> String {
            format!("Welcome back, {} {}!", person.salutation, person.last_name)
        }
    }

    impl Greeter<CasualPerson> for CustomGreeter {
        fn greet(&self, person: &CasualPerson) -> String {
            format!("Hello, {}!", person.name)
        }
    }

    impl Greeter<Anonymous> for CustomGreeter {
        fn greet(&self, person: &Anonymous) -> String {
    ...
    {
        fn greet(&self, person: &AnyPerson) -> String {
...
mod v8 {
    use crate::v3::{Anonymous, CasualPerson, FormalPerson, HasName};
    use crate::v5::{AnyPerson, Either};
    use crate::v6::WordGreeter;
    ...
    pub trait NameGreeter {
        fn greet_name(&self, person: &impl HasName) -> String;
    }
    ...
    fn greet_many<P, G: Greeter<P>>(greeters: &Vec<G>, persons: &Vec<P>) -> Vec<String> {
        greeters
            .iter()
            .map(|greeter| {
                persons
    ...
    pub type AnyGreeterGeneric =
        Either<Unit<PoliteGreeter>, Either<Unit<PersonGreeter<AnyPerson>>, WithName<WordGreeter>>>;
    ...
    #[test]
    fn test() {
        use crate::v5::{make_persons, AnyPerson};
